...
export async function DonatePanelDesktop({ campaignId, title }: { campaignId: string; title: string }) {
  ...
  return (
    <div className="hidden space-y-6 rounded-lg bg-neutral-900 p-6 md:block">
      <h1 className="text-2xl font-extrabold">{title}</h1>
      <HydrationBoundary state={dehydrate(queryClient)}>
        <DonationsProgress campaignId={campaignId} />
      </HydrationBoundary>
      <div>
        <p className="mb-3 font-semibold">Share on:</p>
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <a href="#" className="rounded-lg bg-neutral-800 p-3">
            ...
            <button className="rounded-lg bg-neutral-800 p-3" title="Copy the link">
              <Link size="24" id="link" />
              <Check size="24" className="hidden" id="check" color="green" />
            </button>
          </div>
          <div className="rounded-lg border-2 border-transparent bg-green-500 px-8 py-3 font-bold text-black duration-300 hover:border-green-500 hover:bg-transparent hover:text-white">
            Donate Now
          </div>
        </div>
      </div>
...
export async function DonatePanelMobile({ campaignId, title }: { campaignId: string; title: string }) {
  const queryClient = new QueryClient();
  ...
  return (
    <div className="space-y-6 rounded-lg md:hidden">
      <h1 className="text-2xl font-extrabold">{title}</h1>
      <HydrationBoundary state={dehydrate(queryClient)}>
        <DonationsProgress campaignId={campaignId} />
      </HydrationBoundary>
